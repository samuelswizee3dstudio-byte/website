// Stripe calls this when a product or price changes; it pings the Netlify build
...
export default async (request) => {
  ...
  if (!signingSecret || !buildHook) {
    console.error('Rebuild webhook is not configured (STRIPE_WEBHOOK_SECRET / NETLIFY_BUILD_HOOK_URL).');
  ...
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY ?? 'sk_unused', { maxNetworkRetries: 1 });
